...
public class CDecayA extends JFrame implements ActionListener{
	JPanel redPanel; 
	JLabel label;
	JPanel westPanel;
	JPanel eastPanel;
	...
	CDecayA(){
		...
		c.setVisible(false);
		d.setVisible(false);*/
		header.setText("CALCULATE A OF: A=P(1-i)^n");
		...
		if(e.getSource()==answer) {
			try {
				...
				if(iValue>1||iValue<0) {
					...
					outcome = step2 * pValue;			
					conclusion.setText("accumilated amount is R" + df.format(outcome) + "(using A=P(1-i)^n");
