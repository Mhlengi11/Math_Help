...
public class FutureValueF extends JFrame implements ActionListener {
	JPanel redPanel; 
	JLabel label;
	JPanel westPanel;
	JPanel eastPanel;
		...
		if(e.getSource()==answer) {
			try {
				...
				if(iValue>1||iValue<0) {
					...
					step3 = xValue*(step2-1);
					outcome = step3 / (iValue/tValue);			
					conclusion.setText("Future value is R" + df.format(outcome) + "(using  A=x[(1+1)^n -1]÷i");
