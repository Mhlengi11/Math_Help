...
public class PresentValueP extends JFrame implements ActionListener {
	JPanel redPanel; 
	JLabel label;
	JPanel westPanel;
	JPanel eastPanel;
	...
	PresentValueP(){
		...
		c.setVisible(false);
		d.setVisible(false);*/
		header.setText("CALCULATE F OF:  F=x[1-(1+i)^n]÷i");
		...
		invested.setBounds(0,120,300,20);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource()==answer) {
			try {
				...
				xValue = Double.parseDouble(a.getText());
				...
				if(iValue>1||iValue<0) {
					...
					step3 = xValue*(1-step2);
					outcome = step3 / (iValue/tValue);			
					conclusion.setText("Future value is R" + df.format(outcome) + "(using  A=x[1-(1+i)^n]÷i)");
