...
public class PresentValueN extends JFrame implements ActionListener {
	JPanel redPanel; 
	JLabel label;
	JPanel westPanel;
	JPanel eastPanel;
	...
	PresentValueN(){
		...
		present = new JLabel("Future value:");
		invested = new JLabel("monthly installment:");
		...
		c.setVisible(false);
		d.setVisible(false);*/
		header.setText("CALCULATE n OF:  F=x[(1+i)^n -1]÷i");
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource()==answer) {
			try {
				...
				pValue = Double.parseDouble(a.getText());
				xValue  =Double.parseDouble(b.getText());
				iValue = Double.parseDouble(c.getText());
				tValue = Double.parseDouble(d.getText());
				if((iValue>1||iValue<0) || (pValue < xValue)) {
					JOptionPane.showMessageDialog(null, "interest value is 0≥i≥1", "MATHS HELP", JOptionPane.WARNING_MESSAGE);	
				}
				else {
					step1 = iValue/tValue;
					step2 =(pValue*step1)/xValue;
					step3 =  step1 + 1;
					step4 = 1 - step2;
					outcome = Math.log(step4)/Math.log(step3);			
					conclusion.setText("number of payments are " + -outcome + "(using...");
