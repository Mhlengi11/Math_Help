...
public class Arithmetic extends JFrame implements ActionListener {
	...
	Arithmetic(){
		...
		headerLabel = new JLabel("Enter the first 3 terms of Tn=a+(n-1)d");
		...
		if(e.getSource()==answer) {
			try {
				...
				if((bValue-aValue)!=(cValue-bValue)) {
					JOptionPane.showMessageDialog(null, "This is not an Arithmetic Sequence", "MATHS HELP", JOptionPane.WARNING_MESSAGE);
				}else {
					d = bValue-aValue;
					tn = aValue -d;
					s = 2 * aValue -d;
					sd = d / 2;
					nth.setText("nth term: Tn=" + d + "n+"+tn + "(with n being term number.)");
					sum.setText("sum formula: Sn=" + sd+ "n^2+" + s);
